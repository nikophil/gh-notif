...
test('isRateLimitError : détecte les messages de rate-limit GitHub', () => {
  ...
  assert.equal(isRateLimitError('RATE LIMIT'), true); // insensible à la casse
});

test('isRateLimitError : faux pour les autres erreurs', () => {
...
test('nextBackoffSeconds : 0 → base, sinon double, plafonné', () => {
  ...
  assert.equal(nextBackoffSeconds(400, 60, 600), 600); // plafonné
  assert.equal(nextBackoffSeconds(600, 60, 600), 600); // reste au plafond
});
