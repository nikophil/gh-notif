...
`--serve` ouvre le navigateur au démarrage (`openBrowser`, best-effort) **sauf si `--no-open`**
(option `open: false` de `serve()`) — à utiliser systématiquement pour les smoke tests, sinon
chaque lancement empile un onglet.
