...
37. **Behind column (commits the PR lacks from its base, both PR tables).** Right after
    Branch: `↓N` (muted, tooltip « N commits behind <base> »), sortable (`behind` key,
    first click **desc** = most behind first) and hideable via the §24 gear. Data:
    `baseRef.compare(headRef: "refs/pull/N/head").behindBy`, added **per alias** to the main
    GraphQL batch (`perPr` option of `graphqlPullChunk` — a shared fragment cannot carry the
    PR number) → zero extra request. ⚠️ The **pull ref** is the trick: it lives on the BASE
